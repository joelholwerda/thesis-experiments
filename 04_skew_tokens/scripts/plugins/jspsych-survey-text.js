...
jsPsych.plugins['survey-text'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    display_element.querySelector('#jspsych-survey-text-next').addEventListener('click', function() {
      ...
      // Test whether inputs sum to 100 percent
      // Convert empty responses to 0 for pecentage task
      let testPercentages = Array(input_array.length);
      for (let index = 0; index < input_array.length; index++) {
        if (input_array[index] == "") {
          testPercentages[index] = "0";
        } else {
          testPercentages[index] = input_array[index];
        }
      }

      if (trial.task == "percentage" && (parseInt(testPercentages[0]) + parseInt(testPercentages[1]) + parseInt(testPercentages[2]) != 100)) {
