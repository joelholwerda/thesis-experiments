...
global black white screenRect screenNum MainWindow scr_centre numStimuli stimType stimOrder% variables from the main experiment
global screenID win ftmVal boxVal1 boxVal2 boxVal3 boxVal4 boxVal5 boxVal6 % new global variables
% if strcmp(stimType,'ns') || strcmp(stimType,'NS')
%     global boxVal6
% end
screenID = screenNum;
win = MainWindow;
...
if strcmp(stimType,'high')
    nOuts = 6;
        labelText = {'10 points -->','50 points -->','60 points -->','70 points -->','80 points -->','90 points -->'};

elseif strcmp(stimType,'low')
    nOuts = 6;

        labelText = {'10 points -->','20 points -->','30 points -->','40 points -->','50 points -->','90 points -->'};

end

% Button variables
buttXpos = centerHor + 95;
buttDims = [0 0 85 40];% set size of buttons
buttColors = [150 150 150]'*ones(1,nOuts); % set the box colors
buttOffset = 80; % verticle distance between buttons
buttYpos = 50 + centerVert + ((1:nOuts) - 3)*buttOffset;
for i = 1:length(buttYpos)  % center buttons on 3 points below patient image
    buttRects(:, i) = CenterRectOnPoint(buttDims, buttXpos, buttYpos(i));
end

% Proceed button
proceedDims = [0 0 150 50];
buttYpos = [buttYpos, Vert - 100];
buttColors = [buttColors, [0 0 0]']; % set the Proceed button color
...
instrText1{3} = ' ';
instrText1{4} = 'Enter your answer in the box below the square'; 
instrText1{5} = ' ';
instrText1{6} = 'Click PROCEED when you are finished.';

instrText2{1} = 'When you were presented with this coloured square,';
instrText2{2} = 'on what PERCENTAGE of trials did you experience each';
instrText2{3} = 'of the outcomes listed here?';
instrText2{4} = '  ';
instrText2{5} = 'Click the box next to each outcome to enter a value.';
instrText2{6} = 'If you did not experience an outcome, leave "0" in ';
instrText2{7} = 'the corresponding box.';
instrText2{8} = '  ';
...
for trial = 1:numStimuli
    ...
    proceed2 = 0;
    clickedIn = 0;
    boxVal1 = '0'; boxVal2 = '0'; boxVal3 = '0'; boxVal4 = '0'; boxVal5 = '0'; boxVal6 = '0';
    ...
    while ~proceed2
        ...
        while (clickedIn > 0)
            ...
            if pressed
                ...
                elseif any(allowed == keyInd) && length(currentVal) < (3+blink) % chceck that clicked key a number & the buffer isn't full
                    if any(allowed(11:end) == keyInd) % regular number buttons, rather than numpad buttons
                        keyInd = allowed(find(allowed == keyInd) - 10); % only display the number, not the symbol
                    end
                    if blink % add key to buffer; if blink is on, put the new char before the "_"
            ...
            if buttons(1,1) %left button->choosing
                ...
                if whichNode > 0
                    ...
                    if whichNode == size(buttRects,2) % clicked Proceed button
                        ...
                        elseif nOuts == 6
                            respVals = [str2num(boxVal1), str2num(boxVal2), str2num(boxVal3), str2num(boxVal4), str2num(boxVal5), str2num(boxVal6)];
                        end
...
global win black white ftmVal boxVal1 boxVal2 boxVal3 boxVal4 boxVal5 boxVal6 stimType

Screen(win, 'FillRect', white); % clear screen

for l = 1:length(instrText)
    Screen('DrawText',win,instrText{l},30,50+l*35,black); %draws instructions
end
