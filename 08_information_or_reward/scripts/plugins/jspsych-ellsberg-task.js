    ...
    plugin.trial = function(display_element, trial) {
        ...
        function displayQuestion(value) {
            ...
            question.innerHTML = `
                ...
                One ball will be randomly selected from the box you choose. If the ball is <strong><font color="orange">orange</font color></strong> you will win <strong>100 points</strong>, 
        ...
        function createTable(value){
            ...
            const table = `
                <style type="text/css">
                    ...
                    .tg td{padding:10px 0px;border-style:solid;border-color:#ccc;border-width:0px;border-top-width:1px;border-bottom-width:1px;}
                </style>
                <table class="tg">
                    <tr>
                        <th class="tg-x8tb"></th>
        ...
        function submitSelection(selection){
            ...
            // Record probability of winning and choice
            probWin.push(value);
            choiceArray.push(choice);

            if (ellsbergTrial < trial.transitions.length){

                // Calculate proportion of each colour for next trial
                ...
                // Calculate prize
                let prizeProb = 0; // draw random ball for each trial
                let trialProb = 0; // proportion of orange balls

                for (let trial = 0; trial < choiceArray.length; trial++) {
                    prizeProb = Math.random();

                    // If known option chose, use known probability, else random probability
                    if (choiceArray[trial] == "known") {
                        trialProb = probWin[trial] / 100;
                    } else {
                        trialProb = Math.random();
                    }

                    // Compare with random number to determine prize
                    if (trialProb > prizeProb) {
                        ellsbergArray.push(100);
                    } else {
                        ellsbergArray.push(0);
                    }
                }
