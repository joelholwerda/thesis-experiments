function [finalRespVals, ftmVals, expVals, outLabels, ftmRT, percentRT] = getProbRatings()
...
global screenID win ftmVal expVal boxVal1 boxVal2 boxVal3 boxVal4 boxVal5 boxVal6 boxVal7 boxVal8 boxVal9 % new global variables
...
buttYpos = 100 + centerVert + ((1:5) - 3)*buttOffset;
...
for trial = percentOrder
    ...
    while ~proceed1
        ...
        while (clickedIn > 0)
            ...
            if justClicked
                ...
                currentVal = []; % empty the clicked box
                blink = 0;
                expVal = currentVal; % save current buffer
                startTime = GetSecs;
            end
            ...
            if buttons(1,1) %left button->choosing
                ...
                if whichNode > 0
                    ...
                    if whichNode == size(q1Rects,2) % clicked Proceed button
                        clickedIn = 0; % deactivate the current box
                        % check that 1) they entered something, 2) it isn't just a '-' or '+', and 3) there isn't a '-' or '+' in the middle somewhere
                        if (length(expVal) == 1 && expVal ~= '-' && expVal ~= '+') || (length(expVal) > 1 && (~any(expVal(2:end) == '-') && ~any(expVal(2:end) == '+')))
                            proceed1 = 1;
                        else
                            DrawFormattedText(win, errorText1, 'center', stimImgLoc(4) + 150, errorColor); % draws error message
                        end
    ...
    expVals(trial) = expVal;
    expValNum = str2num(expVal);
...
buttXpos = centerHor + 95;
buttDims = [0 0 85 40];% set size of buttons
buttColors = [240 240 240]'*ones(1,expeValNum); % set the box colors
buttOffset = 80; % verticle distance between buttons
buttYpos = 100 + centerVert + ((1:length(expeValNum)) - 3)*buttOffset;
for i = 1:length(buttYpos)  % center buttons on 3 points below patient image
    buttRects(:, i) = CenterRectOnPoint(buttDims, buttXpos, buttYpos(i));
end
    outLabels(:,trial) = expeValNum(randperm(length(expeValNum))); % randomize outcome order
