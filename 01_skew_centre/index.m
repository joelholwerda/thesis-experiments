...
if strcmp(stimType,'highRank')
    structFile = 'blockStructHighRank.csv';
elseif strcmp(stimType,'lowRank')
    structFile = 'blockStructLowRank.csv';
end
...
numStimuli = 6;                 % Number of stimuli; here coloured shapes
...
% tallyRect = [0, 0, 300, 150];
% tallyWindow = Screen('OpenOffscreenWindow', MainWindow, screenBackgroundColour, tallyRect);
...
% tallyDisplayRect = CenterRectOnPoint(tallyRect, scr_centre(1), 100);
...
for block = 1:numBlocks
    ...
    for trial = 1 : trialPerBlock
        ...
        if strcmp(trialType(trialNum(trial)),'shared')
            trialIndex = 1;
        elseif strcmp(trialType(trialNum(trial)),'context')
            trialIndex = 2;
        elseif strcmp(trialType(trialNum(trial)),'catch')
            trialIndex = 3;
        else
            trialIndex = 4;
        end
...
%         DrawFormattedText(tallyWindow, ['Total Points:\n', num2str(pointTally)], 'center', 'center' , black, [], [], [], 1.5);
...
prizeCalculation = 1; % Calculate prize by random sample (1) or total points (2)
conversionRate = 5000;
